...
pub fn generate(input: TokenStream) -> Result<TokenStream> {
    ...
    let type_params: Vec<Ident> = (0..field_count).map(|i| format_ident!("T{}", i)).collect();
    let param_names: Vec<Ident> = (0..field_count).map(|i| format_ident!("__f{}", i)).collect();
    ...
    let bounds: Vec<TokenStream> = type_params
        ...
        .collect();
    let type_name_exprs: Vec<TokenStream> = type_params
        .iter()
        .map(|t| quote!(#t::TYPE_NAME))
        .collect();
    ...
    let output = if field_count > 0 {
        ...
        quote! {{
            ...
            unsafe {
                ::wpp::etw::write(
                    #provider_mod::STATE.reg_handle(),
                    &__WPP_EVT_DESC, 0, core::ptr::null(),
                );
            }
        }}
    };
