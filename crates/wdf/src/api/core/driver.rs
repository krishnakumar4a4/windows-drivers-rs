...
/// macro attribute.
///
/// `control_blocks` points to the `WPP_MAIN_CB` array generated by the macro,
/// and `num_controls` is the number of trace control entries. When
/// `num_controls == 0`, tracing is not initialised.
#[doc(hidden)]
pub fn call_safe_driver_entry(
    ...
    if num_controls > 0 {
        let trace_writer = unsafe { TraceWriter::init(control_blocks, num_controls, wdm_driver, reg_path) };
...
pub unsafe fn get_auto_log_context() -> Option<PVOID> {
    get_trace_writer().map(|tw| {
        // SAFETY: Union field access. The slice is valid and index 0 exists
        // because tracing was initialised with at least one control block.
...
pub unsafe fn get_wpp_logger() -> Option<TRACEHANDLE> {
    get_trace_writer().map(|tw| {
        // SAFETY: Union field access. Same as get_auto_log_context.
