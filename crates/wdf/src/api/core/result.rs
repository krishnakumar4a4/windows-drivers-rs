...
impl NtStatus {
    pub fn from(code: i32) -> Self {
        if !NT_ERROR(code) {
            Self::NonError(NtStatusNonError::from(code))
        } else {
    ...
    /// Returns true if the status is `NonError`
    pub fn is_non_error(&self) -> bool {
        matches!(*self, Self::NonError(_))
    }
...
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum NtStatusNonError {
    Success(i32),
    Informational(i32),
    Warning(i32),
}

impl NtStatusNonError {
    pub fn from(code: i32) -> Self {
        if NT_INFORMATION(code) {
            ...
            panic!("NtSuccess::from called with non-success code: {:#X}", code);
        } else {
    ...
    /// Returns true if the status a `Success`
    /// Matches the behavior o of the `NT_SUCCESS` macro in C
    pub fn is_success(&self) -> bool {
        matches!(*self, Self::Success(_) | Self::Informational(_))
    }
...
impl Into<i32> for NtStatusNonError {
    fn into(self) -> i32 {
        self.code()
    }
}
...
pub mod status_codes {
    ...
    // Temporarily copied from wdk_sys::constants
    // because it doesn't selectively expose constants
    // (it clubs constants and types together).
    // TODO: Change wdk_sys to expose them independently
    // and then simply re-export them from here
