...
/// Writes the path to the rustc dep-info (`.d`) file into the scratch
/// directory so that the `wdf-macros` proc-macro atexit handler can read
/// the authoritative list of source files that participated in compilation.
///
/// The `.d` file is written by `rustc` after compilation completes and
/// contains every source file that contributed to the build — including
/// files behind `#[cfg]` gates, `include!()`, macro-generated `mod`
/// declarations, and `#[path = "..."]` with arbitrary expressions.
///
/// ## Path derivation
///
/// `OUT_DIR` always has the structure:
///
/// ```text
/// <target_dir>/[<target_triple>/]<profile>/build/<pkg>-<hash>/out
/// ```
///
/// Walking three parents up reaches `<profile>/` (or
/// `<target_triple>/<profile>/` — already accounted for), then appending
/// `deps/` gives us the directory containing the `.d` file.  This works
/// for all combinations of:
///
/// - Custom `CARGO_TARGET_DIR` / `CARGO_BUILD_TARGET_DIR`
/// - Cross-compilation (`--target <triple>`)
/// - Workspace builds (shared `target/` at workspace root)
/// - Custom profiles (`--profile <name>`)
fn write_dep_info_path_hint() {
    use std::path::PathBuf;

    let out_dir = PathBuf::from(
        std::env::var("OUT_DIR").expect("OUT_DIR must be set in build.rs"),
    );
    let crate_name = std::env::var("CARGO_PKG_NAME")
        .expect("CARGO_PKG_NAME must be set in build.rs")
        .replace('-', "_");

    // OUT_DIR = <target_dir>/[<triple>/]<profile>/build/<pkg>-<hash>/out
    //   parent 1: <pkg>-<hash>/
    //   parent 2: build/
    //   parent 3: <profile>/   (or <triple>/<profile>/ — already embedded)
    let deps_dir = out_dir
        .parent() // <pkg>-<hash>/
        .and_then(|p| p.parent()) // build/
        .and_then(|p| p.parent()) // <profile>/
        .expect("OUT_DIR should have at least 3 parent directories")
        .join("deps");

    // For cdylib / bin crates the .d filename has no metadata hash.
    let d_file_path = deps_dir.join(format!("{crate_name}.d"));

    let scratch_dir = scratch::path("wdf_macros_wpp");
    if let Err(e) = std::fs::create_dir_all(&scratch_dir) {
        eprintln!(
            "cargo::warning=dep_info_hint: failed to create scratch dir: {e}"
        );
        return;
    }

    let hint_path = scratch_dir.join("dep_info_path.txt");
    match std::fs::write(&hint_path, d_file_path.to_string_lossy().as_bytes()) {
        Ok(()) => {
            eprintln!(
                "cargo::warning=dep_info_hint: wrote dep-info path hint -> {}",
                hint_path.display()
            );
        }
        Err(e) => {
            eprintln!(
                "cargo::warning=dep_info_hint: failed to write {}: {e}",
                hint_path.display()
            );
        }
    }
